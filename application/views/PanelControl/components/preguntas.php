...
foreach ($preguntas as $key => $value) {
      ...
      // var_dump($sec[0]["seccion"]);
       ...
       if ($value['seccion']==$seccion)
       {
           ...
           <button type="button" class="btn btn-success btn-sm" onclick="return hacerCambio(';echo "'"; echo 'config-pregunta'.$value['id'];echo "'"; echo  ", '".base_url('configUpdatePregunta').$session.'&cat='.strtoupper($categoria).'&idpre='.$value['id'];echo"'"; echo ')"><i class="fa fa-pencil-alt" aria-hidden="true"></i></button>
           <button type="button" class="btn btn-danger btn-sm" onclick="return hacerCambio(';echo "'"; echo 'config-pregunta'.$value['id'];echo "'"; echo  ", '".base_url('configDelPregunta').$session.'&cat='.strtoupper($categoria).'&idpre='.$value['id'];echo"'"; echo ')"><i class="fa fa-trash" aria-hidden="true"></i></button>
           </div>';
           echo '</div>';
